...
assert.ok(plan.workflow.portfolioImpact);
assert.equal(plan.workflow.steps.map((step) => step.id).join(","), "intent_extraction,market_research,portfolio_impact_analysis,risk_sizing,strategy_selection,final_response");
...
assert.match(plan.message, /Step 3 - Portfolio impact:/);
assert.match(plan.message, /Step 4 - Risk sizing:/);
assert.match(plan.message, /Step 5 - Strategy selection:/);
assert.match(plan.message, /Step 6 - Final response:/);
assert.ok(plan.trace.some((step) => step.step === "portfolio_impact_analysis"));
